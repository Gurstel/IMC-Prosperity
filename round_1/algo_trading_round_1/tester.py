...
class Trader:
    ...
    def calc_next_price_starfruit(self):
        ...
        # Total profit: 106,252, 3453.04
        coef = [-0.01852, 0.045501, 0.163, 0.809]
        intercept = 4.485
        nxt_price = intercept
    ...
    def compute_orders_amethysts(self, product, order_depth, acc_bid, acc_ask):
        ...
        if (cpos < self.POSITION_LIMIT["AMETHYSTS"]) and (self.position[product] < -19):
            num = min(20, self.POSITION_LIMIT["AMETHYSTS"] - cpos)
            orders.append(Order(product, min(undercut_buy + 1, acc_bid - 1), num))
            logger.print("Here:", Order(product, min(undercut_buy + 1, acc_bid - 1), num))
            logger.print("Here:", cpos)
            cpos += num
        ...
        if (cpos > -self.POSITION_LIMIT["AMETHYSTS"]) and (self.position[product] > 19) and acc_ask < INF:
    ...
    def compute_orders_regression(self, product, order_depth, acc_bid, acc_ask, LIMIT):
        ...
        if (best_sell_pr + best_buy_pr) / 2 > 5045:
            orders.append(Order(product, 5040, -20 - cpos))
            cpos += -20 - cpos
        elif (best_sell_pr + best_buy_pr) / 2 < 4955:
            orders.append(Order(product, 4960, 20 - cpos))
            cpos += 20 + cpos
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        _, bb_starfruit = self.values_extract(
            collections.OrderedDict(sorted(state.order_depths["STARFRUIT"].buy_orders.items(), reverse=True)),
            1,
        )
