...
class Trader:
    ...
    def orchid_trading(self, state):
        position = state.position.get("ORCHIDS", 0)

        # if we have a position, convert it
        if position < 0:
            logger.print(f"Buying {abs(position)} Orchids From South Arch.")
            self.conversions += abs(position)
        ...
        if obs is not None:
            ...
            orcid_book = state.order_depths["ORCHIDS"]
            buy_orders = orcid_book.buy_orders
            sell_orders = orcid_book.sell_orders

            sells = 0

            if len(buy_orders) != 0:
                bid, size = list(buy_orders.items())[0]  # best bid

                difference = conversion_ask - bid

                total_cost = difference + import_tariff + transport_fee

                logger.print(f"Total Cost: {total_cost}")
                if total_cost < 0:
                    self.send_sell_order("ORCHIDS", bid, -size, msg=f"ORCHIDS: SELL {size} @ {bid}")
                    sells += size

            if len(buy_orders) != 0 and len(sell_orders) != 0:
                bid, _ = list(buy_orders.items())[0]  # best bid
                ask, _ = list(sell_orders.items())[0]  # best ask
                ...
                self.send_sell_order("ORCHIDS", mid_price, -size, msg=f"MARKET MAKE ORCHIDS: SELL {size} @ {mid_price-2}")
