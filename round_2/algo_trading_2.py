...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in ["AMETHYSTS", "STARFRUIT", "ORCHIDS"]:
            if product == "ORCHIDS":
                ...
                orders: List[Order] = []
                orders.append(Order(product, min(state.order_depths[product].sell_orders).keys, 100))
    ...
    def trade_orchids(self, state, orders):
        ...
        # Y=2.0047×Best Mid Price−2.0279×South Mid Price+1.0186×Worst Mid Price+4.1302
        best_mid_price = (best_bid + best_ask) / 2
        south_mid_price = (conversion_ask_price + conversion_bid_price) / 2
        worst_mid_price = (worst_bid + worst_ask) / 2
        sell_price = int(np.round(2.0047 * best_mid_price - 2.0279 * south_mid_price + 1.0186 * worst_mid_price + 4.1302))
