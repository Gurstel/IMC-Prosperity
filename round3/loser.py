...
class Trader:
    ...
    def trade_baskets(self, state, orders):
        ...
        # (signal, limit, hedge)
        buy_combos = [(65, 60), (10, 59)]

        sell_combos = [(99, 60), (44, 60)]
        for level, limit in buy_combos:
            if long_arbitrage_premium > level:
                size = min(-1 * best_basket_ask_volume, abs(limit - basket_position))
                size = min(size, 20)
        ...
        for level, limit in sell_combos:
            if short_arbitrage_premium > level:
                size = min(-1 * best_basket_bid_volume, abs(limit - basket_position))
                size = min(size, 20)
