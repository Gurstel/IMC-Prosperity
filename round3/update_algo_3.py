...
class Trader:
    ...
    def trade_baskets(self, state, orders):
        ...
        if long_arbitrage_premium < premium - 10:
            logger.print("Long arbitrage premium", long_arbitrage_premium)
            size = min(60 - basket_position, best_basket_ask_volume)

            orders.append(Order("GIFT_BASKET", best_basket_bid, size))
            orders.append(Order("STRAWBERRIES", best_strawberry_bid, 6 * size))
            orders.append(Order("CHOCOLATE", best_chocolate_bid, 4 * size))
            orders.append(Order("ROSES", best_rose_bid, size))

        elif short_arbitrage_premium > premium + 270:
            logger.print("Short arbitrage premium", short_arbitrage_premium)
            size = max(-60 - basket_position, best_basket_bid_volume)

            orders.append(Order("GIFT_BASKET", best_basket_ask, size))
            orders.append(Order("STRAWBERRIES", best_strawberry_ask, 6 * size))
            orders.append(Order("CHOCOLATE", best_chocolate_ask, 4 * size))
