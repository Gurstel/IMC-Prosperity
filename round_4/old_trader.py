...
BASKET_PRICE_PERCENT_THRESHOLD = 0.14
BASKET_ORDER_SLOWING_FACTOR = 2

COCONUT_FMV = 10000
COUPON_FMV = 637.63
COUPON_LOADING = 2
COUPON_PRICE_TRADE_PERCENT_THRESHOLD = 0.0
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in ["GIFT_BASKET"]:
            if product == "ORCHIDS":
                ...
                logger.print("ORCHIDS orders", orders)
                ...
                orders, gift_basket_trader_data = self.trade_baskets(state, trader_data)
                for o in orders:
                    if o.symbol not in result:
                        result[o.symbol] = []
                    result[o.symbol].append(o)
                trader_data.update(gift_basket_trader_data)
    ...
    def max_gift_baskets_to_buy(self, state: TradingState):
        gift_baskets_for_sale = sum(-ask_amount for _, ask_amount in list(state.order_depths[GIFT_BASKET].sell_orders.items()))

        strawberris_for_buy = sum(bid_amount for _, bid_amount in list(state.order_depths[STRAWBERRIES].buy_orders.items()))
        chocolates_for_buy = sum(bid_amount for _, bid_amount in list(state.order_depths[CHOCOLATE].buy_orders.items()))
        roses_for_buy = sum(bid_amount for _, bid_amount in list(state.order_depths[ROSES].buy_orders.items()))
    ...
    def max_gift_baskets_to_sell(self, state: TradingState):
        gift_baskets_for_buy = sum(bid_amount for _, bid_amount in list(state.order_depths[GIFT_BASKET].buy_orders.items()))

        strawberris_for_sale = sum(-ask_amount for _, ask_amount in list(state.order_depths[STRAWBERRIES].sell_orders.items()))
        chocolates_for_sale = sum(-ask_amount for _, ask_amount in list(state.order_depths[CHOCOLATE].sell_orders.items()))
        roses_for_sale = sum(-ask_amount for _, ask_amount in list(state.order_depths[ROSES].sell_orders.items()))
    ...
    def trade_baskets(self, state: TradingState, trader_data: dict[str, Any]):
        ...
        prev_avg_gift_basket_premium = trader_data.get("avg_gift_basket_premium", 379)
        prev_count = trader_data.get("count", 1000)
        ...
        if prev_avg_gift_basket_premium is not None:
            ...
            logger.print(f"Real premium: {real_premium}, avg premium: {prev_avg_gift_basket_premium}, pct_change: {pct_change}")
        ...
        new_avg_gift_basket_premium = prev_avg_gift_basket_premium if prev_avg_gift_basket_premium is not None else 0
        new_avg_gift_basket_premium = ((new_avg_gift_basket_premium * prev_count) + real_premium) / new_count
