...
class Trader:
    ...
    def compute_max_coconut_buy_qty(self, state: TradingState):
        # Max quantity of COCONUT that can be bought
        _, best_coupon_bid_qty = self.get_best_bid(state, COUPON)
        _, best_coconut_ask_qty = self.get_best_ask(state, COCONUT)
        return math.floor(
            min(
                [
                    MAX_COCONUT_TRADE_QTY,
                    -best_coconut_ask_qty,
                    POSITION_LIMITS[COCONUT] - state.position.get(COCONUT, 0),
                    best_coupon_bid_qty / COUPON_LOADING,
                    (POSITION_LIMITS[COUPON] + state.position.get(COUPON, 0)) / 2,
                ]
            )
        )

    def compute_max_coconut_sell_qty(self, state: TradingState):
        # Max quantity of COCONUT that can be sold
        _, best_coupon_ask_qty = self.get_best_ask(state, COUPON)
        _, best_coconut_bid_qty = self.get_best_bid(state, COCONUT)
        return math.floor(
            min(
                [
                    MAX_COCONUT_TRADE_QTY,
                    best_coconut_bid_qty,
                    POSITION_LIMITS[COCONUT] + state.position.get(COCONUT, 0),
                    -best_coupon_ask_qty / COUPON_LOADING,
                    (POSITION_LIMITS[COUPON] - state.position.get(COUPON, 0)) / 2,
                ]
            )
        )
    ...
    def trade_coconuts(self, state, orders):
        best_coconut_bid, best_coconut_bid_qty = self.get_best_bid(state, COCONUT)
        best_coconut_ask, best_coconut_ask_qty = self.get_best_ask(state, COCONUT)
        best_coupon_bid, best_coupon_bid_qty = self.get_best_bid(state, COUPON)
        best_coupon_ask, best_coupon_ask_qty = self.get_best_ask(state, COUPON)

        coconut_midprice = (best_coconut_bid + best_coconut_ask) / 2

        expected_coupon_price = (coconut_midprice - COCONUT_FMV) * compute_coconut_coupon_delta(coconut_midprice)[1] + COUPON_FMV
        ...
        if (best_coupon_bid - expected_coupon_price) / expected_coupon_price > TRADE_PERCENT_THRESHOLD:
            # Sell COUPON and buy COCONUT
            qty = self.compute_max_coconut_buy_qty(state)
            orders.append(Order(COCONUT, best_coconut_ask, qty))
            orders.append(Order(COUPON, best_coupon_bid, -qty * COUPON_LOADING))

        elif (best_coupon_ask - expected_coupon_price) / expected_coupon_price < -TRADE_PERCENT_THRESHOLD:
            # Buy COUPON and sell COCONUT
            qty = self.compute_max_coconut_sell_qty(state)
            orders.append(Order(COUPON, best_coupon_ask, qty * COUPON_LOADING))
            orders.append(Order(COCONUT, best_coconut_bid, -qty))
