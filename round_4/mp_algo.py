...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in ["AMETHYSTS", "STARFRUIT", "ORCHIDS", "GIFT_BASKET", "COCONUT"]:
            if product == "ORCHIDS":
                ...
                result["COCONUT"] = [o for o in orders if o.symbol == "COCONUT"]
                result["COCONUT_COUPON"] = [o for o in orders if o.symbol == "COCONUT_COUPON"]
    ...
    def trade_coupons(self, state, orders):
        ...
        if len(coconut_buy_orders.keys()) < 1 or len(coconut_sell_orders.keys()) < 1 or len(coupon_sell_orders.keys()) < 1 or len(coupon_buy_orders) < 1:
            return
